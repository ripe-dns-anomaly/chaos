	...
	if not res.get('error'):
		...
		if data[index]['result'].get('answers'):
			...
			if isinstance(data[index]['result']['answers'][0]['RDATA'], list):
				f.write('%s %s %s %s\n' % (data[index]['prb_id'], data[index]['result']['answers'][0]['RDATA'][0], data[index]['result']['rt'], str(x.rcode)))
			else:
				f.write('%s %s %s %s\n' % (data[index]['prb_id'], data[index]['result']['answers'][0]['RDATA'], data[index]['result']['rt'], str(x.rcode)))
